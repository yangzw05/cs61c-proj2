...
int pass_one(FILE* input, FILE* output, SymbolTable* symtbl) {
    ...
    while (fgets(buf, BUFSIZ, input)!=NULL) {
        ...
        if (token == NULL) {
            continue;       //if nothing in the line, go to next line
        }
        int val = add_if_label(input_line, token, byte_offset, symtbl);
        if (val==-1) {err = -1;}
        char name[15]; 
        char* args[MAX_ARGS];
        /* scan text after label, if nothing, go to next line*/
        if (val!=0) {         
        ...
        if (extra_arg_err ==0) {
            int inst_lines = write_pass_one(output, name, args, num_args);
            if (inst_lines == 0) {
                raise_inst_error(input_line, name, args, num_args);
                err = -1;
            } else {
                byte_offset += inst_lines * 4;
            }
        }          
    }
    return err;
...
int pass_two(FILE *input, FILE* output, SymbolTable* symtbl, SymbolTable* reltbl) {
    ...
    // raise_inst_error(). 
        int val = translate_inst(output, name, args, num_args, byte_offset, symtbl, reltbl);
        byte_offset += 4;
        if (val == -1) {
            raise_inst_error(input_line, name, args, num_args);
            err = -1;
